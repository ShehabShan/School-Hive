...
- [x] **F1 [P0/Bug] Home stats API 500** — desktop `/` console: `Failed to load resource: 500` on `GET https://server-six-vert.vercel.app/allScholership/stats`. Mobile run clean (flaky or viewport-triggered). Fix: check server stats controller/route (likely in `Schole-hive-server`), add error guard + client fallback so home renders stats section without console error. Verify: load `/` desktop, no 500, no console error.
  - **Root cause found:** `getStats` `byCountry` pipeline nested `$sort`/`$limit` inside the `$group` stage object (invalid stage → Mongo throws → 500); plus `$toDouble` on dirty legacy strings. **Fixed on server `testing`:** separate pipeline stages + `$convert {onError:0,onNull:0}`, `ObjectId.isValid` guard in `getScholarshipById` (invalid id → 404, was 500 throw). Client `useScholarshipStats` now `retry: 1`. **Needs deploy to verify live.**
- [x] **F2 [P1/Bug] Unsplash hotlink fails on scholarship lists** — `/scholarships` + `/allScholership` (both viewports): `requestfailed https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=800&q=80`. Page still renders H1, so likely one card/banner image. Fix: self-host under `public/` or add `onError` fallback + `loading="lazy"`. Verify: no `requestfailed`, no broken-image icon.
  - **Root cause:** URL comes from DB data (`universityImage`), not code — no code match for it. **Fixed:** `Pages/AllScholership/ScholarshipCard.jsx` img now `onError` → placehold.co fallback (shared `Component/scholarship/ScholarshipCard.jsx` already had it). Bad DB value still worth replacing with self-hosted image later.
- [ ] **F3 [P1/Perf] `/contact`, `/aboutUs`, `/compare` ~33s + empty H1** — vs ~3s for all other routes, both viewports; H1 empty (slow lazy chunk or blocking fetch, Suspense fallback stuck). Fix: inspect lazy imports + data fetching on `ContactPage`, `AboutUs`, `Compare`; preconnect/preload or remove blocking await. Verify: each < 5s on desktop, meaningful H1/heading present.
  - **Re-probe: NOT reproduced** — isolated re-run + instrumented run show `/contact` goto 683ms, all chunks < 500ms, curl TTFB < 1.5s all paths. Earlier 30–60s was transient (hosting/network), not code. Minor leftovers (no code change): those pages have no H1 (contact h2 `Contact Us` only; compare empty without `?ids=`); `/clear-jwt` took 1.7s once (Vercel cold start). No fix applied.
- [x] **F4 [P2/UX] 404 page is a dead end (no nav)** — `/nonexistent-xyz-123`: H1 `Page not found` correct, but `navLinks 0` (no header/nav rendered, unlike 7 links on every other page). Fix: render `NotFound` inside `MainLayout` or add Home/browse links. Verify: 404 shows nav + working Home link.
  - **Fixed:** added `{ path: "*", element: <NotFound /> }` under `/` (MainLayout) so unknown URLs render with header/nav; `errorElement` kept. `NotFound` itself already had Home/Browse links.
- [x] **F5 [P2/UX] Common guessed URLs 404** — `/allScholarships` (double-p), `/login`, `/register`, `/qa` all show `Page not found` (correct per current routes; first sweep confirmed). Canonical: `/scholarships` (also `/allScholership` typo-alias exists), `/signIn`, `/registration`, `/questions`. Fix (optional): add redirect aliases in `Routes.jsx` for the guessed names. Verify: each alias lands on canonical page, no 404.
  - **Fixed:** `Navigate` aliases in `Routes.jsx` — `/login`→`/signIn`, `/register`→`/registration`, `/qa`→`/questions`, `/allScholarships`→`/allScholership`. Client `lint 0`.
