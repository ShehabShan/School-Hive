...
## 2026-09-07 — Profile + Notifications + Threaded Replies Polish (P1, N1–N4b, Q1–Q4) — Completed Batch Moved

> **Source:** `TASKS.md: TODO — Profile + Notifications + Threaded Replies Polish (2026-09-07 — P1)` — all 10 checked per archive rule `TASKS.md:10`, moved to keep TASKS lean. **Resolutions folded in (2026-09-07):** (1) both bells share `useNotifications` cache — not a move; (2) mute 2 options only — thread + type, no actor; (3) cover+overlap on public too — flat was bug; (4) replies no rep; (5) denormalized `commentCount`. Built `P1`→`Q4` dependency-ordered: `P1 || N1/Q1` → `N1→N2→N3→N4a→N4b` + `Q1→Q2→Q3→Q4` (Q4 last to account for nested UI).
...
- [x] **P1. [Bug] Fix avatar-over-cover overlap + restore cover on public profiles** — `ProfileLayout.jsx:22,37-40` + `ProfileHeaderQuora.jsx:54` flat. Fix: `pt-px` + `relative z-10 isolate`, audit `overflow-hidden` clipping, re-add cover `hasValue(user.coverPhoto)?user.coverPhoto:bg` + `relative h-44` + `-mt-10` to Quora. Orphans deprecated. Verify: 320/768/1280 overlap 40–56px, ring-4 visible. **DONE `ProfileLayout.jsx:37-38` pt-px + isolate, `ProfileHeaderQuora.jsx:58-76` cover restored**
- [x] **N1. [Feature] Extract shared `useNotifications` data source** — `NotificationBell.jsx:32` → `useNotifications.jsx` (`enabled !!user`, `staleTime 30s`, `refetchOnWindowFocus`, `GET /notifications/me?limit=10`). Verify: single cache. **DONE `useNotifications.jsx` created, `NotificationBell.jsx:3-26` delegates**
- [x] **N2. [Feature] Add notification bell to main site nav (keep dashboard bell — both visible)** — `Nabvar.jsx:172` `Ask → Bell → avatar`, gated `!!user`, `unread>9`. Keep `AdminNavbar.jsx:119`. Both share `N1` cache. Verify: bell on `/`, `/allScholership`, `/questions/:id` and dashboards same count. **DONE `Nabvar.jsx:4,175-179`**
- [x] **N3. [Feature] Clean up notification system (while in N1/N2)** — align `limit=10` (`notification.controller.js:10` `15/max 50`), bypass 30s LRU for `GET /notifications/me` (`app.js:55`), polish a11y `role=dialog/menu/menuitem`, keep dupe window. Verify: uncached, no duplicate fetches. **DONE `cache.js:3` bypass `/notifications`, `NotificationBell.jsx:64-80` roles**
- [x] **N4a. [Feature] Server: mute preferences store + filter (question + type only)** — `users.notificationMutes {mutedTypes[], mutedQuestionIds[]}`, `PATCH /notifications/preferences/mute` + `unmute`, filter in `createNotification` early-return + `listMine` `{type:$nin, payload.questionId:$nin}`. No actor mute. Verify: mute type/thread hides future inserts. **DONE `notification.service.js:7`, `notification.controller.js:9/23/45`, `notification.routes.js:5`**
- [x] **N4b. [Feature] Client: per-notification kebab menu — mute this thread / mute this type** — `Ellipsis` kebab per row (hover/focus), 2 actions only: Mute this question (default) + Mute this type, no person, wire to `N4a` PATCH, toast Undo. Verify: `⋯` → mute thread/type, undo restores, keyboard nav. **DONE `NotificationBell.jsx:3-156` kebab + mute mutations**
- [x] **Q1. [Feature] Server: `comments` collection — nested replies on answers + denormalized `commentCount`, no reputation** — `collections.comments` indexed `(answerId,parentId,questionId)`, backfill `answers.commentCount 0`, validator `1–2000`, `buildCommentDoc {questionId,answerId,parentId,rootId,depth}`, routes `POST /answers/:id/comments` (inc `commentCount`), `GET /answers/:id/comments`, `PATCH/DELETE /comments/:id` (dec). No `reputationEvents`. Notify `comment_reply`. Verify: depth 0 inc, reply depth1, GET flat, DELETE dec. **DONE `comment.validator.js`, `comment.controller.js`, `comment.routes.js`, `db.js:24/38`, `answer.validator.js`**
- [x] **Q2. [Feature] Client: `CommentThread` + reply composer under each answer** — `QuestionDetail.jsx:487` map `<AnswerCard>` + `<CommentThread>`, `CommentThread` fetch `GET /answers/:id/comments`, tree build, `CommentItem` + `ReplyComposer` (1–2000, `useAxiosSecure`), collapsible count badge, invalidates `["comments",answerId]` + `["question",id]`. Verify: reply count, inline editor, instant, `commentCount` inc. **DONE `CommentThread.jsx`, `CommentItem.jsx`, `ReplyComposer.jsx`, `QuestionDetail.jsx:5,487`**
- [x] **Q3. [Feature] Cap nesting at 3 levels, flatten deeper replies** — `renderDepth = min(depth,3)`, indent `ml-4/8/10-12`, `border-l`, flattened label `Replying to @parent`. Server stores true depth, client caps visual. Verify: chain 0→5 indented to 3 then 4/5 same as 3, no 320px overflow. **DONE `CommentItem.jsx:12-18` visualDepth, `CommentThread.jsx:35` parentAuthorEmail**
- [x] **Q4. [Feature] Responsive redesign of `QuestionDetail` `motion.article` `src/Pages/QA/QuestionDetail.jsx:349` — desktop/tablet/mobile (after Q2/Q3)** — `flex-col sm:flex-row`, mobile pill `sm:hidden` + desktop rail `hidden sm:flex w-[84px]`, `p-4 sm:p-6`, `prose-sm sm:prose-base`, tags `gap-1.5`, `min-w-0`, skeleton updated. Verify: 320/375/768/1024/1280 no scroll, thumb-reachable, `lg:grid-cols-[1fr_340px]` intact. **DONE `QuestionDetail.jsx:38-65` skeleton, `350-421` article responsive**
