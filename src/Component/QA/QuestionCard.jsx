...
export function QuestionListItem({ q }){
  ...
  return (
    <Link to={`/questions/${q._id}`} className="group flex gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all hover:border-brand-300 hover:shadow-md">
...
export function QuestionCard({ q }){
  const excerpt = stripMarkdown(q.body);
  return (
    <Link to={`/questions/${q._id}`} className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all hover:border-brand-300 hover:shadow-md">
      <div className="flex items-start justify-between gap-2">
        <h3 className="line-clamp-2 text-[15px] font-bold leading-snug text-slate-900 group-hover:text-brand-600">{q.title}</h3>
        <AnswerStat count={q.answerCount ?? 0} accepted={Boolean(q.acceptedAnswerId)} />
      </div>
      {excerpt && <p className="mt-1 line-clamp-3 text-sm text-slate-500">{excerpt.slice(0, 180)}</p>}
